/*
Problem 8 from Project Euler.  http://projecteuler.net/problem=8
...
// This map will store the result of every multiplication we do so that we can
// quickly look up previous results instead of repeating calculations.
var products = make(map[pairInt]int)

var numMults int
var numLookups int

/*
GetProduct calculates the product of the values in nums and returns that value.
*/
func GetProduct(nums []int) int {
	...
	case len(nums) == 1:
		return nums[0]
	case len(nums) > 2:
		p = pairInt{GetProduct(nums[0:2]), GetProduct(nums[2:len(nums)])}
	case len(nums) == 2:
		// Always store the pair in sorted order to reduce lookups
		if nums[0] < nums[1] {
			p = pairInt{nums[0], nums[1]}
		} else {
			p = pairInt{nums[1], nums[0]}
		}
	}

	if product, ok := products[p]; ok {
		numLookups++
		return product
	}
...
func main() {
	...
	for j < len(digits) {
		// If the fifth digit in the group is a zero move the entire group
		// forward to one past the 0.
		if digits[j-1] == 0 {
			i = j
			j = i + 5
			continue
		...
		// Calculate the product of the 5 digits in the current group.
		product = GetProduct(digits[i:j])

		if product > biggest {
			biggest = product
		}

		// Move the group forward by one place.
		i++
		j++
	}
